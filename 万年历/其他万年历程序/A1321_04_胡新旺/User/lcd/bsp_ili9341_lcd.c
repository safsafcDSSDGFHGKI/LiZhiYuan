...
/**
 * @brief  在 ILI9341 显示器上显示一个英文字符(32*16)
 * @param  usX ：在特定扫描方向下字符的起始X坐标
 * @param  usY ：在特定扫描方向下该点的起始Y坐标
 * @param  cChar ：要显示的英文字符
 * @param  usColor_Background ：选择英文字符的背景色
 ...
 * @retval 无
 */
void ILI9341_DispChar_EN_32 ( uint16_t usX, uint16_t usY, const char cChar, uint16_t usColor_Background, uint16_t usColor_Foreground )
{
	uint8_t ucTemp, ucRelativePositon, ucPage, ucColumn;
	...
	ILI9341_OpenWindow ( usX, usY, 16, 32 );
	
	ILI9341_Write_Cmd ( macCMD_SetPixel );	
	
	for ( ucPage = 0; ucPage < 32; ucPage ++ )
	{
		ucTemp = ucAscii_3216 [ ucRelativePositon ] [ ucPage ] [ 0 ];
		...
		ucTemp = ucAscii_3216 [ ucRelativePositon ] [ ucPage ] [ 1 ];
...
/**
 * @brief  在 ILI9341 显示器上显示一个英文字符(48*24)
 * @param  usX ：在特定扫描方向下字符的起始X坐标
 * @param  usY ：在特定扫描方向下该点的起始Y坐标
 * @param  cChar ：要显示的英文字符
 * @param  usColor_Background ：选择英文字符的背景色
 ...
 * @retval 无
 */
void ILI9341_DispChar_EN_48 ( uint16_t usX, uint16_t usY, const char cChar, uint16_t usColor_Background, uint16_t usColor_Foreground )
{
	uint8_t ucTemp, ucRelativePositon, ucPage, ucColumn;
	...
	ILI9341_OpenWindow ( usX, usY, 24, 48 );
	
	ILI9341_Write_Cmd ( macCMD_SetPixel );	
	
	for ( ucPage = 0; ucPage < 48; ucPage ++ )
	{
		ucTemp = ucAscii_4824 [ ucRelativePositon ] [ ucPage ] [ 0 ];
		...
		ucTemp = ucAscii_4824 [ ucRelativePositon ] [ ucPage ] [ 1 ];
		...
		ucTemp = ucAscii_4824 [ ucRelativePositon ] [ ucPage ] [ 2 ];
...
/**
 * @brief  在 ILI9341 显示器上显示英文字符串(32*16)
 * @param  usX ：在特定扫描方向下字符串的起始X坐标
 * @param  usY ：在特定扫描方向下字符串的起始Y坐标
 * @param  pStr ：要显示的英文字符串的首地址
 * @param  usColor_Background ：选择英文字符串的背景色
 ...
 * @retval 无
 */
void ILI9341_DispString_EN_32 ( uint16_t usX, uint16_t usY, const char * pStr, uint16_t usColor_Background, uint16_t usColor_Foreground )
{
	while ( * pStr != '\0' )
	{
		if ( ( usX - macILI9341_DispWindow_X_Star + 16 ) > macILI9341_DispWindow_COLUMN )
		{
			usX = macILI9341_DispWindow_X_Star;
			usY += 32;
		}
		
		if ( ( usY - macILI9341_DispWindow_Y_Star + 32 ) > macILI9341_DispWindow_PAGE )
		{
			usX = macILI9341_DispWindow_X_Star;
			usY = macILI9341_DispWindow_Y_Star;
		}
		
		ILI9341_DispChar_EN_32 ( usX, usY, * pStr, usColor_Background, usColor_Foreground );
...
/**
 * @brief  在 ILI9341 显示器上显示英文字符串(48*24)
 * @param  usX ：在特定扫描方向下字符串的起始X坐标
 * @param  usY ：在特定扫描方向下字符串的起始Y坐标
 * @param  pStr ：要显示的英文字符串的首地址
 * @param  usColor_Background ：选择英文字符串的背景色
 ...
 * @retval 无
 */
void ILI9341_DispString_EN_48 ( uint16_t usX, uint16_t usY, const char * pStr, uint16_t usColor_Background, uint16_t usColor_Foreground )
{
	while ( * pStr != '\0' )
	{
		if ( ( usX - macILI9341_DispWindow_X_Star + 24 ) > macILI9341_DispWindow_COLUMN )
		{
			usX = macILI9341_DispWindow_X_Star;
			usY += 48;
		}
		
		if ( ( usY - macILI9341_DispWindow_Y_Star + 48 ) > macILI9341_DispWindow_PAGE )
		{
			usX = macILI9341_DispWindow_X_Star;
			usY = macILI9341_DispWindow_Y_Star;
		}
		
		ILI9341_DispChar_EN_48 ( usX, usY, * pStr, usColor_Background, usColor_Foreground );
...
//	
//	/*画键盘背景*/
//	ILI9341_Clear(20,140,200,160,macBLUE2);
//	/*画键盘分割线*/
//	for(i=0;i<4;i++)
//      ILI9341_Clear(18+i*65,138,4,164,macGREY);   //竖线
//	 for(i=0;i<5;i++)
//      ILI9341_Clear(20,138+i*40,200,4,macGREY);   //横线
//	/* 键盘显示0 */
//	ILI9341_DispChar_EN(100,275,0x30,macBLUE2,macRED);
//	/* 键盘显示1~9 */
//	for(i=1;i<10;i++)
//	{
//	     ILI9341_DispChar_EN(40+x,155+y,0x30+i,macBLUE2,macRED);
//	     x+=65;
//		 if(x>140)
//		{  x=0;  y+=40;  }	 
//	}
//     ILI9341_DispString_CH(40,275,"清除",macBLUE2,macRED);
//	 ILI9341_DispString_CH(160,275,"确定",macBLUE2,macRED);
//}
