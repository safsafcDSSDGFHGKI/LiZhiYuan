...
strType_XPT2046_TouchPara strXPT2046_TouchPara = //{ 0.085958, -0.001073, -4.979353, -0.001750, 0.065168, -13.318824 };  //初始值为扫描方式2下的一组校准系数，不校准触摸屏时可使用该组默认系数
                                               //{ 0.001030, 0.064188, -10.804098, -0.085584, 0.001420, 324.127036 };  //初始值为扫描方式1下的一组校准系数，不校准触摸屏时可使用该组默认系数
																							 { 0.001576, 0.066243, -15.917830, -0.090625, 0.001101, 330.387012 };  //自己测试的较准扫描方式1下参数

volatile uint8_t ucXPT2046_TouchFlag = 0;

XPT2046_Coordinate  tp_dev;
...
{	
	...
	if( ucXPT2046_TouchFlag )		//有按键按下
	{
		if ( XPT2046_Get_TouchedPoint ( & Coordinate, & strXPT2046_TouchPara ) )
		{
			tp_dev.x = Coordinate.x;
			tp_dev.y = Coordinate.y;
		}
		...
		else tp_dev.sta |= TP_CATH_PRES;
	}
	else
	{
		if(tp_dev.sta&TP_PRES_DOWN)//之前是被按下的
		{
			tp_dev.sta &= ~TP_PRES_DOWN;//刚释放
		}
		else//完全释放
		{
			tp_dev.sta = 0;
